...
class CoastlineGenerator {
  ...
  private sourceKey(feature: Feature): string {
    return JSON.stringify([feature.type, feature.coastline, feature.vertices.map(vertex => pack.vertices.p[vertex])]);
  }
  ...
  getFeaturePath(feature: Feature): string {
    ...
    const path = `${round(this.buildPath(this.getFeatureShape(feature, outline)))}Z`;
