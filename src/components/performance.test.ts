...
describe("presets", () => {
  it("names the settings a fresh browser has", () => {
    expect(resolvePerformancePreset(options.app.performance)).toBe("speed"); // the fork defaults to redraw after zoom
  });
  ...
  it("ignores a name that is not a preset", () => {
    ...
    expect(options.app.performance).toEqual(PERFORMANCE_PRESETS.speed);
  });
});
