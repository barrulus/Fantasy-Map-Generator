...
const STATES_SORT_ACCESSORS = {
  name: (s: any) => s.name,
  form: (s: any) => s.formName,
  capital: (s: any) => (s.i ? pack.burgs[s.capital].name : ""),
  culture: (s: any) => (s.i ? pack.cultures[s.culture].name : ""),
  burgs: (s: any) => s.burgs,
  area: (s: any) => s.area,
  population: (s: any) => s.rural * populationRate + s.urban * populationRate * urbanization,
  type: (s: any) => s.type || "",
  expansionism: (s: any) => s.expansionism || 0,
  cells: (s: any) => s.cells
};
...
function statesEditorAddLines(): void {
  ...
  renderEditorPagination(ensureEl("statesFooter"), pageInfo, (page: number) => {
...
function populateBrushStateSelect(): void {
  ...
  sel.innerHTML = states
    .map((s: any) => `<option value="${s.i}">${s.i ? s.fullName : s.name}</option>`)
    .join("");
}
...
function stageStateDemotion(stateId: number): void {
  ...
  for (let i = 0; i < cellState.length; i++) {
    ...
    if (existing.size())
      existing.attr("data-state", ownerId).attr("data-demote", stateId).attr("fill", demoteColor).attr("stroke", demoteColor);
    else
      temp
        .append("polygon")
        .attr("data-cell", i)
  ...
  tip(`${(pack.states as any)[stateId].fullName} staged to become a province of ${(pack.states as any)[ownerId].name}. Apply to commit`, true);
}
...
function openStateMergeDialog(): void {
  ...
  function highlightStateOnMergeHover(event: any) {
    ...
    if (!state) return;
    const stateNode = regions.select(`#state${state}`).node() as SVGPathElement | null;
...
function mergeStates(statesToMerge: number[], rulingStateId: number, mergeToProvinces = false): void {
    ...
    function demoteStateToProvince(stateId: number): void {
      ...
      (pack.provinces as any[]).push({i: newProvinceId, state: rulingStateId, center, burg, name, formName, fullName, color: provinceColor, coa, pole});
...
function downloadStatesCsv(): void {
  ...
  const data = pack.states
    ...
    .map(s => {
      ...
      const capital = s.i ? (pack.burgs as any)[s.capital].name : "";
      const culture = s.i ? (pack.cultures as any)[s.culture].name : "";
      return [
        s.i,
        s.name,
        s.fullName || "",
